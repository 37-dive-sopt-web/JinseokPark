...
- **`selectAllCheckbox()`**
  ...
  - **반환 값:** 없음 (DOM 요소 직접 조작)
  - **간단한 로직**
    1. 전체 체크박스의 상태에 따라, 아래 체크박스도 동일하게 설정합니다. (checked인지 아닌지)
    2. 전체 체크박스를 제외한 아래 체크박스 중 몇 개가 선택되었는지에 따라 전체 체크박스 상태를 바꿉니다.
...
- 그리드 구조 사용 (3개의 `column`이 일정하게 공간을 나눠가지도록 설정)

#### 3.2 필터 섹션

- 그리드 구조 사용

  1. 우선 `row`를 설정하고, `input`이 2개 또는 3개 들어가야 하는 부분은 따로 `column` 설정.

  2. 각 `label`과 `input`의 레이아웃도 그리드 활용. (`label`이 `4rem` 공간 차지, `input`은 나머지 공간 모두 차지)

#### 3.3 테이블 섹션

- `section` 태그 안에 2개의 div로 구현
  1. 첫 번째 `div`는 제목과 버튼 2개 포함
  2. 두 번째 `div`가 `table` 태그 포함 - `thead`만 설정해두고, `tbody`는 비워둔 후 자바스크립트로 조작 (반응형을 고려하여 테이블을 감싸는 `div`에서 가로 스크롤을 처리)
- `colgroup` 활용해서 테이블의 각 열의 너비 설정

#### 3.4 모달

- 한 줄에 `input`이 2개 들어가야 하는 경우에는 그리드 구조 활용
- 배경으로 사용할 `div` 안에 모달 `div` 구현 - 배경 어두워지는 효과 및 눌렀을 때 모달 닫히는 기능 고려
